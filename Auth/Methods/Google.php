...
use PhalconRest\Exceptions\UserException,
	...
	PhalconRest\Models\GoogleAccounts;
...
class AuthGoogle extends \Phalcon\Mvc\User\Plugin
{
	...
	public function codeValid($code)
	{
		...
		if (isset($code)){
			...
			try {
				...
				throw new UserException(ErrorCodes::AUTH_BADLOGIN, 'Expired Google login');
			}
	...
	public function getUserInfo()
	{
		...
		try {
			...
			throw new CoreException(ErrorCodes::GOOGLE_NODATA, 'Could not get user data');
		}
	...
	protected function register($payload, $userinfo)
	{
		...
		if (!$googleAccount->save()){
		    ...
		    throw new CoreException(ErrorCodes::USER_REGISTERFAIL, 'GoogleAccount for user #' . $user->id . ' could not be created');
		}
